...
class Labelme2YOLO(object):
    ...
    def __init__(self, json_dir, obj_names, output_path):
        self._json_dir = json_dir
        self._obj_names = obj_names
        if output_path:
            self._output_path = output_path
        else:
            self._output_path = os.path.join(self._json_dir, 'YOLODataset')
        
        self._label_id_map = self._get_label_id_map(self._json_dir)
        
    def _make_train_val_dir(self):
        self._label_dir_path = os.path.join(self._output_path, 
                                            'labels/')
        self._image_dir_path = os.path.join(self._output_path, 
                                            'images/')
        ...
        for file_name in os.listdir(json_dir):
            if file_name.endswith('json'):
                ...
                for shape in data['shapes']:
                    if shape['label'] in self._obj_names:       ## modified for specific object
                        label_set.add(shape['label'])

        label_list = sorted(label_set)      # sort the label list
    ...
    def judge_obj_names(self, json_data):
        if not json_data['shapes']:
            return True
        for shape in json_data['shapes']:
            # labelme circle shape is different from others
            # it only has 2 points, 1st is circle center, 2nd is drag end point
            if shape['label'] in self._obj_names:
                return True
        return False
    ...
    def convert(self, val_size):
        ...
        for target_dir, json_names in zip(('train/', 'val/'), 
            ...
            for json_name in json_names:
                ...
                if not self.judge_obj_names(json_data):
                    continue
                count += 1
        ...
        print(f'Totoally {count} labeled images.')
    ...
    def _get_yolo_object_list(self, json_data, img_path):
        ...
        for shape in json_data['shapes']:
            # labelme circle shape is different from others
            # it only has 2 points, 1st is circle center, 2nd is drag end point
            if shape['shape_type'] == 'circle' and shape['label'] in self._obj_names:   ## modified for specific obj
    ...
    def _save_yolo_image(self, json_data, json_name, image_dir_path, target_dir):
        ...
        if not os.path.exists(img_path):
            try:
                ...
                print(f'save_img {img_path}')
                orig_imgpath = os.path.join(self._json_dir, img_name)
                img = PIL.Image.open(orig_imgpath)
                img.save(img_path)
    ...
    def _save_dataset_yaml(self):
        yaml_path = os.path.join(self._output_path, 'dataset.yaml')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--obj_names', type=str, default='B,D,H')
    parser.add_argument('--output_path', type=str, default=None)
    args = parser.parse_args(sys.argv[1:])
        
    obj_names= args.obj_names.split(',')
    convertor = Labelme2YOLO(args.json_dir, obj_names, args.output_path)
